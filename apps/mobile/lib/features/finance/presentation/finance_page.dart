      ...
      child: ListView(
        ...
        children: [
          Row(children: [
            ...
            Expanded(
                child: _Kpi(
                    ...
                    value: s.profitPerDelivery == null
                        ? '—'
          ...
          if (state.insights.hasData) ...[
            NavixSectionHeader(
                title: l10n.finInsights, icon: Icons.insights_outlined),
            _InsightsCard(insights: state.insights),
            const SizedBox(height: 12),
          ],
          NavixSectionHeader(
              title: l10n.finEntries, icon: Icons.receipt_long_outlined),
          if (state.entries.isEmpty)
            Padding(
              padding: const EdgeInsets.symmetric(vertical: 24),
              child: Text(l10n.finNoEntries,
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              if (suffix != null)
                Text(suffix!, style: TextStyle(fontSize: 11.5, color: t.muted)),
            ],
          ),
        ],
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return NavixCard(
      ...
      child: Row(children: [
        ...
        cell(l10n.finBalance, summary.balance,
            summary.balance >= 0 ? t.success : t.danger),
      ]),
    );
  }
}
...
class _PeriodBars extends StatelessWidget {
  ...
  double _h(double v) =>
      max <= 0 ? 4 : (4 + (v / max) * 64).clamp(4, 68).toDouble();
  ...
  Widget build(BuildContext context) {
    ...
    final label = parts.length >= 3
        ? '${parts[2]}/${parts[1]}'
        : (parts.length == 2 ? parts[1] : point.period);
    return Padding(
...
class _HourBars extends StatelessWidget {
  const _HourBars(
      {required this.byHour, required this.color, required this.track});
              ...
              child: Padding(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: counts[h] == 0
                        ? track
                        : (peak ? color : color.withValues(alpha: 0.55)),
          ...
          children: [
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(financeCategoryLabel(l10n, entry.category),
                      style: const TextStyle(
            ...
            Text('$sign € ${entry.amount.toStringAsFixed(2)}',
                style: TextStyle(
                    fontSize: 14.5, fontWeight: FontWeight.w700, color: color)),
            IconButton(
              tooltip: l10n.finDelete,
              onPressed: () =>
                  context.read<FinanceCubit>().deleteEntry(entry.id),
