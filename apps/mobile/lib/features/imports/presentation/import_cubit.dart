...
class ImportState extends Equatable {
  ...
  List<Object?> get props =>
      [step, busy, preview, confirmation, history, historyLoading, error];
}
...
class ImportCubit extends Cubit<ImportState> {
  ...
  Future<void> pickAndPreview(
    ...
    try {
      ...
      emit(state.copyWith(
          busy: false, step: ImportStep.preview, preview: preview));
    } on Failure catch (f) {
  ...
  Future<void> confirm() async {
    ...
    try {
      ...
      emit(state.copyWith(
          busy: false, step: ImportStep.done, confirmation: result));
      await loadHistory();
    } on Failure catch (f) {
      emit(state.copyWith(busy: false, error: f));
    } catch (_) {
