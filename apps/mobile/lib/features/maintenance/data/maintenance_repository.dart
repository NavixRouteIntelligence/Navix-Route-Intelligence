...
class MaintenanceRepository {
  ...
  Future<MaintenanceVehicle?> myVehicle() async {
    try {
      final res = await _dio
          .get<dynamic>('/fleet/vehicles', queryParameters: {'pageSize': 1});
  ...
  Future<List<MaintenanceRecord>> records(String vehicleId) async {
    try {
      final res =
          await _dio.get<dynamic>('/fleet/vehicles/$vehicleId/maintenance');
  ...
  Future<List<MaintenanceReminder>> reminders(String vehicleId) async {
    try {
      final res = await _dio
          .get<dynamic>('/fleet/vehicles/$vehicleId/maintenance/reminders');
  ...
  Future<void> addRecord(String vehicleId, NewMaintenanceRecord record) async {
    try {
      await _dio.post<dynamic>('/fleet/vehicles/$vehicleId/maintenance',
          data: record.toJson());
    } on DioException catch (e) {
      throw mapDioException(e);
    }
  ...
  Future<void> updateOdometer(String vehicleId, int odometerKm) async {
    try {
      await _dio.patch<dynamic>('/fleet/vehicles/$vehicleId',
