...
class TrackedDriver extends Equatable {
  ...
  List<Object?> get props =>
      [id, name, status, latitude, longitude, speedKmh, recordedAt, plate];
}

/// Um ponto do histórico de posições (timeline).
class TrackPoint extends Equatable {
  const TrackPoint(
      {required this.recordedAt, required this.status, this.speedKmh});
...
class FleetAlert extends Equatable {
  const FleetAlert(
      {required this.id, required this.severity, required this.message});
