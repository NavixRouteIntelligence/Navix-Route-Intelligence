...
class EarningsState {
  const EarningsState(
      {this.tariff = const DriverTariff(), this.loaded = false});

  final DriverTariff tariff;
  final bool loaded;

  EarningsState copyWith({DriverTariff? tariff, bool? loaded}) => EarningsState(
      tariff: tariff ?? this.tariff, loaded: loaded ?? this.loaded);
}
...
class EarningsCubit extends Cubit<EarningsState> {
  ...
  Future<void> save(
      {required double perDelivery, required double perKm}) async {
