        ...
        builder: (context, state) {
          final child = switch (state.status) {
            ...
            DashboardStatus.error => NavixErrorState(
                description: context.failureText(
                ...
                Row(
                  children: [
                    NavixDonut(
                      ...
                      segments: [
                        DonutSegment(
                          data.deliveries.delivered.toDouble(),
                          t.success,
                        ),
                        DonutSegment(
                          data.deliveries.inRoute.toDouble(),
                          primary,
                        ),
                        DonutSegment(
                          data.deliveries.pending.toDouble(),
                          t.warning,
                        ),
                        DonutSegment(
                          data.deliveries.failed.toDouble(),
                          t.danger,
                        ),
                      ],
                    ),
...
class _Legend extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final t = context.tokens;
    Widget item(Color c, String l) => Row(
      ...
      child: Row(
        children: [
          ...
          Column(
            ...
            children: [
              Text(
                '−${plan.savingsPct.toStringAsFixed(0)}% km',
                style: TextStyle(
                  color: t.accent,
      ...
      children: [
        ...
        GridView.count(
          ...
          children: List.generate(
            4,
            (_) => const NavixCard(child: NavixSkeleton(height: 60)),
          ),
        ),
        const SizedBox(height: 16),
