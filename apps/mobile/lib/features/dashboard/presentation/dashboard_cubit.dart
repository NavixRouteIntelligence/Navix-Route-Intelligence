...
class DashboardState extends Equatable {
  const DashboardState(
      {this.status = DashboardStatus.loading, this.data, this.error});
  ...
  DashboardState copyWith(
          {DashboardStatus? status, DashboardData? data, Failure? error}) =>
      DashboardState(
          status: status ?? this.status, data: data ?? this.data, error: error);
  ...
  Future<void> load() async {
    ...
    try {
      ...
      emit(const DashboardState(
          status: DashboardStatus.error, error: UnknownFailure()));
    }
  }
}
