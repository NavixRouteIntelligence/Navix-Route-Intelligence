...
class DashboardRepository {
  ...
  Future<DashboardData> load() async {
    try {
      final deliveries = _map(await _dio
          .get<dynamic>('/deliveries', queryParameters: {'pageSize': 100}));
      final plans = _map(await _dio
          .get<dynamic>('/route-plans', queryParameters: {'pageSize': 50}));
      final pod = _map(await _dio.get<dynamic>('/pod/summary'));
      final positions =
          _map(await _dio.get<dynamic>('/tracking/positions/latest'));
      final imports = _map(await _dio
          .get<dynamic>('/imports', queryParameters: {'pageSize': 5}));
      ...
      final vehicles = _map(await _dio
          .get<dynamic>('/fleet/vehicles', queryParameters: {'pageSize': 100}));
      final drivers = _map(await _dio
  ...
  int _avgScore(List<Map<String, dynamic>> items) {
    ...
    final sum =
        items.fold<int>(0, (a, p) => a + ((p['score'] as num?)?.toInt() ?? 0));
  ...
  List<double> _series(List<Map<String, dynamic>> items,
  ...
  FleetCounts _fleet(
      Map<String, dynamic> vehicles, Map<String, dynamic> drivers) {
