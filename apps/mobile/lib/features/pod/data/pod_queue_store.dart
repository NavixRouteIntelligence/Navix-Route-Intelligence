...
class QueuedPod {
  const QueuedPod(
      {required this.id, required this.createdAt, required this.submission});
...
class PodQueueStore {
  PodQueueStore({Directory Function()? overrideDir})
  ...
  Future<Directory> _dir() async {
    final base =
        _overrideDir?.call() ?? await getApplicationDocumentsDirectory();
  ...
  Future<void> enqueue(PodSubmission submission) async {
    ...
    final id =
        '${DateTime.now().millisecondsSinceEpoch}_${submission.deliveryId}';
    final payload = {
      'id': id,
      ...
      try {
        ...
        out.add(QueuedPod(
          ...
          createdAt: DateTime.tryParse((json['createdAt'] as String?) ?? '') ??
              DateTime.now(),
          submission:
              PodSubmission.fromStorage(json['pod'] as Map<String, dynamic>),
        ));
