...
class NavixSplashPainter extends CustomPainter {
  ...
  void _paintTrace(Canvas canvas, Path route, double stroke) {
    ...
    canvas.drawPath(
      ...
      Paint()
        ...
        ..strokeCap = StrokeCap.round
        ..strokeJoin = StrokeJoin.round
        ..shader = ui.Gradient.linear(bounds.bottomLeft, bounds.topRight, [
          palette.primary,
          palette.accent,
        ]),
    );
  ...
  void _paintArrow(Canvas canvas, Offset center, double side) {
    ...
    canvas.drawPath(
      popped,
      Paint()..color = palette.accent.withValues(alpha: reveal.clamp(0.0, 1.0)),
    );
  }
  ...
  void _paintParticles(Canvas canvas, Offset center, double side, double t) {
    ...
    for (var i = 0; i < particleCount; i++) {
      ...
      paint.color = (i.isEven ? palette.accent : Colors.white).withValues(
        alpha: alpha,
      );
