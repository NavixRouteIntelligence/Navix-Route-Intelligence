...
void main() {
  testWidgets('descartar sem nunca ter animado não lança', (tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(
          body: NavixPulseDot(color: Colors.green, animate: false),
        ),
      ),
    );
    await tester.pump();
  ...
  testWidgets('alternar animate liga e desliga o pulso sem lançar', (
    tester,
  ) async {
    Widget host(bool animate) => MaterialApp(
          home: Scaffold(
            body: NavixPulseDot(color: Colors.green, animate: animate),
          ),
        );
