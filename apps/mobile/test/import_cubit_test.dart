...
void main() {
  ...
  const preview = ImportPreview(
    batch: ImportBatch(
      ...
      summary: ImportSummary(
          ...
          invalid: 1,
          duplicates: 1,
          estimatedSavingsKm: 12,
          estimatedSavingsPct: 15),
    ),
    rows: [
      ImportRow(
          index: 0, status: ImportRowStatus.valid, addressText: 'Rua A, 100'),
      ImportRow(
          index: 1,
  ...
  const confirmation =
      ImportConfirmation(createdDeliveries: 8, routePlanId: 'plan-1');
  const history = [
    ImportBatch(
        id: 'batch-1',
        filename: 'rotas.csv',
        ...
        summary: ImportSummary(total: 10, valid: 8)),
  ];
  ...
  blocTest<ImportCubit, ImportState>(
    ...
    build: () {
      when(() => repo.preview(
          path: any(named: 'path'),
          filename: any(named: 'filename'))).thenAnswer((_) async => preview);
  ...
  blocTest<ImportCubit, ImportState>(
    ...
    build: () {
      ...
      when(() => repo.list(pageSize: any(named: 'pageSize')))
    ...
    expect: () => [
      ...
      const ImportState(
          step: ImportStep.done, preview: preview, confirmation: confirmation),
      const ImportState(
