  ...
  group('EarningsCubit', () {
    ...
    test('load: lê a tarifa do armazém e marca loaded', () async {
      when(() => store.read()).thenAnswer(
          (_) async => const DriverTariff(perDelivery: 2, perKm: 0.5));
      final cubit = EarningsCubit(store);
      await cubit.load();
      expect(cubit.state.loaded, isTrue);
      expect(
          cubit.state.tariff, const DriverTariff(perDelivery: 2, perKm: 0.5));
    });
    ...
    test('save: persiste e atualiza o estado, saneando negativos', () async {
      ...
      expect(
          cubit.state.tariff, const DriverTariff(perDelivery: 0, perKm: 0.7));
      verify(() => store.save(const DriverTariff(perDelivery: 0, perKm: 0.7)))
          .called(1);
    });
  });
}
