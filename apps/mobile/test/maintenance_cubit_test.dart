...
void main() {
  ...
  const vehicle = MaintenanceVehicle(
      id: 'v1', plate: 'AA-00-BB', type: 'car', odometerKm: 120000);
  const record = MaintenanceRecord(
      id: 'm1', type: 'oil_change', performedAt: '2026-07-01');
  const reminder = MaintenanceReminder(
      type: 'insurance', status: 'due_soon', remainingDays: 16);
  ...
  blocTest<MaintenanceCubit, MaintenanceState>(
    ...
    expect: () => [
      const MaintenanceState(status: MaintenanceStatus.loading),
      const MaintenanceState(
          status: MaintenanceStatus.error, error: NetworkFailure()),
    ],
  );
