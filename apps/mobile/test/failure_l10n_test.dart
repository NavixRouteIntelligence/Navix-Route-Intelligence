...
Future<String> _text(WidgetTester tester, Failure failure,
...
void main() {
  group('localizedMessage', () {
    testWidgets('toda falha tem texto nos 5 locales suportados',
        (tester) async {
    ...
    testWidgets(
        'traduz por locale — nada de português fixo para quem usa en/es',
        (tester) async {
      expect(
          await _text(tester, const NetworkFailure(),
    ...
    testWidgets('pt-PT usa o vocabulário europeu, distinto de pt-BR',
        (tester) async {
      final pt = await _text(tester, const CacheFailure(),
          locale: const Locale('pt', 'PT'));
      final br = await _text(tester, const CacheFailure(),
          locale: const Locale('pt', 'BR'));
      expect(pt, contains('guardados'));
      expect(br, contains('salvos'));
    });

    testWidgets('credencial inválida não fala em sessão expirada',
        (tester) async {
      final invalid = await _text(tester, const InvalidCredentialsFailure(),
          locale: const Locale('en'));
      final expired = await _text(tester, const UnauthorizedFailure(),
          locale: const Locale('en'));
      expect(invalid, 'Incorrect email or password.');
      expect(invalid, isNot(expired));
    });

    testWidgets('detalhe do servidor prevalece sobre o texto genérico',
        (tester) async {
      expect(await _text(tester, const ServerFailure('Rota já otimizada.')),
          'Rota já otimizada.');
      expect(await _text(tester, const ValidationFailure('CEP inválido.')),
          'CEP inválido.');
    });

    testWidgets('sem detalhe do servidor, cai no texto localizado do tipo',
        (tester) async {
      expect(
          await _text(tester, const ServerFailure(),
              locale: const Locale('en')),
          "The server couldn't complete the request.");
      expect(
          await _text(tester, const ValidationFailure(),
              locale: const Locale('en')),
          "Some of the data isn't valid.");
    });

    testWidgets('cada motivo de localização tem a sua orientação',
        (tester) async {
      final texts = {
        for (final r in LocationErrorReason.values)
          r: await _text(tester, LocationFailure(r),
              locale: const Locale('en')),
      };
      expect(
          texts.values.toSet(), hasLength(LocationErrorReason.values.length));
      expect(
          texts[LocationErrorReason.permissionBlocked], contains('Settings'));
    });
  });
}
