...
void main() {
  group('localizedMessage', () {
    ...
    testWidgets('pt-PT usa o vocabulário europeu, distinto de pt-BR', (
      ...
      final pt = await _text(
        tester,
        const CacheFailure(),
        locale: const Locale('pt', 'PT'),
      );
      final br = await _text(
        tester,
        const CacheFailure(),
        locale: const Locale('pt', 'BR'),
      );
    ...
    testWidgets('credencial inválida não fala em sessão expirada', (
      ...
      final invalid = await _text(
        tester,
        const InvalidCredentialsFailure(),
        locale: const Locale('en'),
      );
      final expired = await _text(
        tester,
    ...
    testWidgets('detalhe do servidor prevalece sobre o texto genérico', (
      ...
      expect(
        await _text(tester, const ServerFailure('Rota já otimizada.')),
        'Rota já otimizada.',
      );
      expect(
        await _text(tester, const ValidationFailure('CEP inválido.')),
        'CEP inválido.',
      );
    });
    ...
    testWidgets('sem detalhe do servidor, cai no texto localizado do tipo', (
      ...
      expect(
        await _text(tester, const ServerFailure(), locale: const Locale('en')),
    ...
    testWidgets('cada motivo de localização tem a sua orientação', (
      ...
      expect(
        texts.values.toSet(),
        hasLength(LocationErrorReason.values.length),
      );
      expect(
        texts[LocationErrorReason.permissionBlocked],
        contains('Settings'),
      );
    });
  });
