...
export class KpiRepository implements KpiRepositoryPort {
  ...
  async rebuildDay(tenantId: string, day: string): Promise<void> {
    await this.manager.query(
      ...
      INSERT INTO kpi_daily (
        tenant_id, day, plans, saved_km, saved_minutes, optimized_km, baseline_km, score_sum,
        delivered, failed, canceled, on_time, expense, updated_at
      )
      SELECT $1, $2::date, p.plans, p.saved_km, p.saved_minutes,
             p.optimized_km, p.baseline_km, p.score_sum,
      ...
      ON CONFLICT (tenant_id, day) DO UPDATE SET
        ...
        baseline_km = EXCLUDED.baseline_km,
  ...
  async range(tenantId: string, from: string, to: string): Promise<KpiDailyRow[]> {
    const rows = (await this.manager.query(
      `SELECT day::text, plans, saved_km, saved_minutes, optimized_km, baseline_km, score_sum,
    ...
    return rows.map((r) => ({
      ...
      savedMinutes: Number(r.saved_minutes),
      optimizedKm: Number(r.optimized_km),
      baselineKm: Number(r.baseline_km),
